...
static void fprint_str(FILE * f, node_string str)
{
    fprintf(f, "%.*s\n", (int) str->len, str->buf);
}
...
static void print_id(const char *pre, node_string name)
{
    fputs(pre, stdout);
    print_str(name);
}

static void print_quoted_id(const char *pre, node_string name)
{
    fputs(pre, stdout);
    fputs("\"", stdout);
    print_str(name);
...
int main(int argc, const char **argv)
{
    ...
    if (n == 0) {
        ...
        if (check) {
            ...
            for (i = 0; i < argc; i++) {
                buf[i] =
                    strm_str_value(strm_str_new(argv[i], strlen(argv[i])));
            }
