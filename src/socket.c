...
static int accept_cb(strm_task * task, strm_value data)
{
...
#endif
    strm_io_emit(task,
                 strm_ptr_value(strm_io_new
                                (sock,
                                 STRM_IO_READ | STRM_IO_WRITE |
                                 STRM_IO_FLUSH)), sd->sock, accept_cb);
    return STRM_OK;
}

static int server_accept(strm_task * task, strm_value data)
{
    struct socket_data *sd = task->data;
...
static int server_close(strm_task * task, strm_value d)
{
    struct socket_data *sd = task->data;
...
static int
tcp_server(strm_state * state, int argc, strm_value * args,
           strm_value * ret)
{
    ...
    sd->state = state;
    task =
        strm_task_new(strm_task_prod, server_accept, server_close,
                      (void *) sd);
...
static int
tcp_socket(strm_state * state, int argc, strm_value * args,
           strm_value * ret)
{
    struct addrinfo hints;
    struct addrinfo *result, *rp;
