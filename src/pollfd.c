...
#endif
    _epoll_events =
        realloc(_epoll_events,
                sizeof(struct epoll_event) * _num_epoll_events);
    memset(&_epoll_events[num], 0, sizeof(struct epoll_event));
    for (i = 0; i < FD_SETSIZE; i++)
        _epoll_events[num].fds[i] = -1;
    return num;
}
        ...
        for (i = 0; i < FD_SETSIZE; i++) {
            ...
            if (WSAEnumNetworkEvents((SOCKET) ee->fds[i], NULL, &wev) == 0) {
                ...
                if (ee->events & EPOLLIN)
                    FD_SET(ee->fds[i], &fdset[0]);
                if (ee->events & EPOLLOUT)
                    FD_SET(ee->fds[i], &fdset[1]);
                if (ee->events & EPOLLERR)
                    FD_SET(ee->fds[i], &fdset[2]);
