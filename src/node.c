...
node *node_op_new(const char *op, node * lhs, node * rhs)
{
...
node *node_call_new(node_string ident, node * recv, node * args,
                    node * blk)
{
...
static size_t string_escape(char *s, size_t len)
{
    char *t = s;
    char *tend = t + len;
    char *p = s;
...
node *node_string_new(const char *s, size_t len)
{
...
node_string node_str_new(const char *s, size_t len)
{
    node_string str;
...
node_string node_str_escaped(const char *s, size_t len)
{
    len = string_escape((char *) s, len);
...
node *node_break_new(node * value)
{
