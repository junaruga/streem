...
static int io_push(int fd, strm_task * task, strm_callback cb)
{
...
static int io_kick(int fd, strm_task * task, strm_callback cb)
{
...
void strm_io_start_read(strm_task * task, int fd, strm_callback cb)
{
...
static void strm_io_stop(strm_task * task, int fd)
{
...
static strm_value read_str(const char *beg, size_t len)
{
    char *p = malloc(len);
...
static int read_cb(strm_task * task, strm_value data)
{
...
static int readline_cb(strm_task * task, strm_value data)
{
    ...
    /* no newline */
        ...
        if (task->flags & STRM_IO_NOWAIT) {
            strm_task_push(strm_queue_task
                           (task, read_cb, strm_nil_value()));
        } else {
...
static int stdio_read(strm_task * task, strm_value data)
{
    struct fd_read_buffer *b = task->data;
...
static int read_close(strm_task * task, strm_value d)
{
    struct fd_read_buffer *b = task->data;
...
static strm_task *strm_readio(strm_io io)
{
    ...
    if (io->read_task == NULL) {
...
#ifdef STRM_IO_MMAP
            buf->buf =
                mmap(NULL, st.st_size, PROT_READ, MAP_PRIVATE, buf->fd, 0);
        ...
        io->read_task =
            strm_task_new(strm_task_prod, cb, read_close, (void *) buf);
...
static int write_cb(strm_task * task, strm_value data)
{
...
static int write_close(strm_task * task, strm_value data)
{
    struct write_data *d = (struct write_data *) task->data;
...
static strm_task *strm_writeio(strm_io io)
{
    struct write_data *d;
    ...
    if (io->write_task) {
        ...
        d->rc = 1;
        io->write_task =
            strm_task_new(strm_task_cons, write_cb, write_close,
                          (void *) d);
    }
