...
strm_value strm_cfunc_value(strm_cfunc f)
{
...
strm_value strm_int_value(int32_t i)
{
    return STRM_TAG_INT | ((uint64_t) i & STRM_VAL_MASK);
}

strm_value strm_flt_value(double f)
{
    union {
        double f;
        uint64_t i;
...
static inline int32_t strm_to_int(strm_value v)
{
    return (int32_t) (v & ~STRM_TAG_MASK);
}

int strm_flt_p(strm_value v)
{
    /* confirm for nan/+inf/-inf */
    return v == STRM_TAG_NAN || (v & STRM_TAG_NAN) != STRM_TAG_NAN;
}

static inline double strm_to_flt(strm_value v)
{
    union {
        double f;
        uint64_t i;
...
int32_t strm_value_int(strm_value v)
{
    switch (strm_value_tag(v)) {
    case STRM_TAG_INT:
...
double strm_value_flt(strm_value v)
{
    if (strm_int_p(v)) {
...
int strm_value_eq(strm_value a, strm_value b)
{
    if (a == b)
        return TRUE;
    if (strm_value_tag(a) != strm_value_tag(b))
        return FALSE;
    ...
    case STRM_TAG_CFUNC:
        return (strm_cfunc) strm_value_val(a) ==
            (strm_cfunc) strm_value_val(b);
    case STRM_TAG_PTR:
...
static size_t str_dump_len(strm_string str)
{
...
static strm_string str_dump(strm_string str, size_t len)
{
    char *buf = malloc(len);
    char *s = buf;
    ...
    if (strm_string_p(v)) {
        ...
        for (i = 0; i < strm_ary_len(a); i++) {
            ...
            strm_string key = (strm_ary_headers(a) &&
                               strm_string_p(strm_ary_ptr
                                             (strm_ary_headers(a))[i])) ?
                strm_value_str(strm_ary_ptr(strm_ary_headers(a))[i]) :
                strm_str_null;
            size_t slen =
                (key ? (strm_str_len(key) + 1) : 0) + strm_str_len(str) +
                3;
