...
static int task_tid(strm_task * s, int tid)
{
    int i;
...
static void task_push(int tid, struct strm_queue_task *t)
{
    strm_task *s = t->strm;
...
void strm_emit(strm_task * task, strm_value data, strm_callback func)
{
...
int strm_task_connect(strm_task * src, strm_task * dst)
{
    ...
    if (src->mode == strm_task_prod) {
        ...
        strm_task_push(strm_queue_task
                       (src, src->start_func, strm_nil_value()));
    }
    return STRM_OK;
}
...
strm_task *strm_task_new(strm_task_mode mode, strm_callback start_func,
                         strm_callback close_func, void *data)
{
...
static int pipeline_finish(strm_task * task, strm_value data)
{
    pthread_mutex_lock(&pipeline_mutex);
    pipeline_count--;
    if (pipeline_count == 0) {
...
void strm_task_close(strm_task * task)
{
    if (task->close_func) {
    ...
    while (d) {
        strm_task_push(strm_queue_task
                       (d, (strm_callback) strm_task_close,
                        strm_nil_value()));
        d = d->nextd;
    }
    if (task->mode == strm_task_prod) {
        strm_task_push(strm_queue_task
                       (task, pipeline_finish, strm_nil_value()));
    }
}
