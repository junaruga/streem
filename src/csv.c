...
static int count_fields(strm_string line)
{
...
static strm_value csv_string(const char *p, size_t len, int ftype)
{
    strm_string str;
...
static strm_value csv_value(const char *p, size_t len, int ftype)
{
    const char *s = p;
...
static int csv_accept(strm_task * task, strm_value data)
{
    strm_array ary;
    ...
    if (cd->prev) {
        ...
        *(tmp + strm_str_len(cd->prev)) = '\n';
        memcpy(tmp + strm_str_len(cd->prev) + 1, strm_str_ptr(line),
               strm_str_len(line));
            ...
            for (i = 0; i < fieldcnt; i++) {
                if (cd->types[i] != csv_type(strm_ary_ptr(ary)[i])) {
                    if (cd->types[i] == STRING_TYPE) {
                        ...
                        ((strm_value *) strm_ary_ptr(ary))[i] =
                            strm_str_value(strm_to_str
                                           (strm_ary_ptr(ary)[i]));
                    } else {
...
static int csv_finish(strm_task * task, strm_value data)
{
    struct csv_data *cd = task->data;
...
csv(strm_state * state, int argc, strm_value * args, strm_value * ret)
{
    ...
    task =
        strm_task_new(strm_task_filt, csv_accept, csv_finish, (void *) cd);
